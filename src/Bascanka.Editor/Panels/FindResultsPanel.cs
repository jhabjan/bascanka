...
/// </summary>
public sealed class NavigateToResultEventArgs : EventArgs
{
    /// <summary>The search result the user wants to open.</summary>
    public SearchResult Result { get; }

    public NavigateToResultEventArgs(SearchResult result)
    {
...
public class FindResultsPanel : UserControl
{
    ...
    /// </summary>
    private sealed class LoadPageMarker
    {
    ...
    // ── State ────────────────────────────────────────────────────────
    private readonly List<SearchSession> _sessions = new();
        ...
        try
        {
            ...
            if (rootNode.Nodes.Count > 0)
            {
                TreeNode target = forward
                    ? rootNode.Nodes[0]
                    : rootNode.Nodes[rootNode.Nodes.Count - 1];
    ...
    private ContextMenuStrip BuildContextMenu()
    {
        ...
        _menuCopyLine.ShortcutKeyDisplayString = "Ctrl+C";
